...
let areHoursInvalid h = if h < 1 || h > 12 then true
                        else false

// This function checks if a minute value `m` is not in [0,59] range
let areMinutesInvalid m = if m < 0 || m > 59 then true
                          else false
...
let time h m ampm :(int * int * AMPM) =
    let h = if areHoursInvalid h then 12
    let m = if areMinutesInvalid m then m = 0
    (h, m, ampm)
